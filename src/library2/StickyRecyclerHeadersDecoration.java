...
public class StickyRecyclerHeadersDecoration extends RecyclerView.ItemDecoration {
	...
	public void onDrawOver(Canvas canvas, RecyclerView parent, RecyclerView.State state) {
		...
		if (parent.getChildCount() > 0 && mAdapter.getItemCount() > 0) {
			...
			if (nextPosition != -1 && hasNewHeader(nextPosition)) {
				...
				// place, if applicable
				if (orientation == LinearLayoutManager.VERTICAL && nextView.getTop() - secondHeader.getHeight() - firstHeader.getHeight() < 0) {
					translationY = nextView.getTop() - secondHeader.getHeight() - firstHeader.getHeight();
				} else if (orientation == LinearLayoutManager.HORIZONTAL && nextView.getLeft() - secondHeader.getWidth() - firstHeader.getWidth() < 0) {
			...
			canvas.restore();
			mHeaderRects.put(firstPosition, new Rect(translationX, translationY, translationX + firstHeader.getWidth(), translationY + firstHeader.getHeight()));

			if (parent.getChildCount() > 1)
				for (int i = 1; i < parent.getChildCount(); i++) {
					int position = parent.getChildPosition(parent.getChildAt(i));
					if (hasNewHeader(position)) {
						// this header is different than the previous, it must
						...
						header.draw(canvas);
						canvas.restore();
						mHeaderRects.put(position, new Rect(translationX, translationY, translationX + header.getWidth(), translationY + header.getHeight()));
					}
				}
		}
	}
	...
	/**
	 * Returns the first item currently in the recyclerview that's not obscured
	 * by a header.
	 * 
	 * @param parent
	 * @return
